...
class Game {
  ...
  public Game() {
    ...
    worldMap.stream()
        .forEach((arr) -> IntStream.range(0, N).forEach((j) -> arr.add(new LinkedList<>())));
    horses.values().stream()
        .forEach((horse) -> worldMap.get(horse.getX()).get(horse.getY()).addLast(horse));
  }
  ...
  private boolean mv_finish(int idx) {
    ...
    if (horse != worldMap.get(horse.getX()).get(horse.getY()).getFirst()) {
      return false;
    }
    mv(horse);
  ...
  private void mv(Horse horse) {
    ...
    while (!worldMap.get(x).get(y).isEmpty()) {
      Horse t_horse = worldMap.get(x).get(y).removeFirst();
      temp.addFirst(t_horse);
      t_horse.setX(tx);
      t_horse.setY(ty);
    }
